...
def post(request):
    ...
    post = Post(
        author=request.user, content=content, timestamp=datetime.datetime.now(), likes=0
    )
    post.save()
...
def like(request, post_id):
    ...
    post.save()
    return JsonResponse(
        {"message": "Like Success", "status": 200, "likes": post.likes}, status=200
    )
...
def isliked(request, post_id):
    ...
    like_exists = Like.objects.filter(user=request.user, post=post).exists()
    return JsonResponse(
        {"message": "Like Success", "status": 200, "isliked": like_exists}, status=200
    )
...
def follow(request):
    ...
    follow_object = Follow(follower=request.user, following=following_user)
    follow_object.save()
...
def unfollow(request):
    ...
    already_exists = Follow.objects.filter(
        follower=request.user, following=unfollowing_user
    ).exists()
    if not already_exists:
        return JsonResponse(
            {"error": "Cannot unfollow a user you don't follow"}, status=400
        )
